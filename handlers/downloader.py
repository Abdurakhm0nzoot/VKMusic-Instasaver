...
async def handle_url(message: Message, bot: Bot) -> None:
    ...
    # Basic duplicate detection for the same user/url
    url_key = f"{message.from_user.id}:{url}"
    if url_key in _in_progress:
        return
    _in_progress.add(url_key)

    platform = detect_platform(url)
    if not platform:
        _in_progress.remove(url_key)
        return  # Not a supported platform

    # Get user info (cached via database)
    ...
    # From now on, use 'finally' to ensure url_key is removed from _in_progress
    status_msg = None
    try:
        ...
        # ── Persistent URL Cache for buttons ──
        async with async_session() as session:
            url_hash = await save_url_cache(session, url)
        ...
        if cached_file_id:
            try:
                ...
                await bot.send_video(
                    ...
                    reply_markup=video_actions_kb(url_hash, lang),
                )
                if status_msg:
            ...
            if file_id:
                async with async_session() as session:
                    await cache_service.save_file_id(
                        ...
                        telegram_file_id=file_id, file_type="video",
        ...
        # Crucial: remove from in-progress so user can send next link
        if url_key in _in_progress:
            _in_progress.remove(url_key)
